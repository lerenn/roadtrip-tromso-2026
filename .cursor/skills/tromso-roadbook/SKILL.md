...
3. Every ferry crossing should have a miss contingency with `"attach": "ferry_crossing"`. Every `overnight.type: "scenic"` night should have a `campsite_alt_*` scenario with `"attach": "overnight"`; every `overnight.type: "campsite"` night should have a `wild_alt_*` scenario with `"attach": "overnight"` (see Contingencies).
...
- **`kind: "via"` / bare `kind: "start"`** = map/routing pin only (no timeline row). Drive steps merge across vias; a morning `start` is only the departure pin so the chronology opens on the first real Drive/stop (e.g. Day 5 skips “Bleik (Midnattsol Camping)” as a line but keeps it on the map). Use `viewpoint` / `depot` / `ferry` when the place should also be a StepCard.
...
- Day-level: `day.scenarios[]` — top-of-day checkboxes unless line-attached (`attach: "ferry_crossing"` or `attach: "overnight"`).
...
- **Wild overnight → campsite:** every `overnight.type: "scenic"` night should have a scenario (id like `campsite_alt_*`, `"attach": "overnight"`) that `replace`s `overnight` with a real `type: "campsite"` object (geocoded lat/lon, `url` / `maps`, `reserve: true` when bookable) and updates the final sleep waypoint (plus a short via if the campsite is a meaningful detour). UI toggle lives on the Overnight StepCard (like ferry miss). Default stays wild (unchecked). Prefer campsites named in `alt_campsite` or already used elsewhere on the trip; never invent coords. Ripple the next day’s start when overnight moved far enough that the default start is wrong (e.g. Hamn → Finnsnes, Nyksund → Stø).
- **Campsite overnight → wild:** every `overnight.type: "campsite"` night should have the mirror scenario (id like `wild_alt_*`, `"attach": "overnight"`) that `replace`s `overnight` with a real `type: "scenic"` object (geocoded lat/lon, `maps`, usually no `url`/`price`/`reserve`). UI toggle on the Overnight StepCard. Default stays campsite (unchecked). Prefer the place hinted in `alt_scenic`; if vague, pick the best named pull-off / Scenic Route parking near that night and say so in notes. Clear campsite leftovers via null (`url`, `reserve`, `price`, `alt_scenic`) and set `alt_campsite` back to the facilities default. Ripple the next day’s start when the wild pin makes the default start wrong (e.g. Tungeneset vs Camp Mefjord, Telegrafbukta vs Tromsø Lodge).
- Optionals: all off by default; selection shifts later starts and marks them with `*` (`applyOptionalSelection`). Prefer separate optionals over line-level `fallback` blocks. Do not add whole-day “bad weather” scenarios — weather choices live as ordinary optionals.
- Selected optionals with `lat`/`lon` reshape the day map / GPX via `routingWaypoints`. Plain same-stop activities (already ~400 m from a must waypoint, e.g. Hamn kayak) do not duplicate pins. **`reserve` activities** at an existing stop still get a labelled pin (activity name) so bookable quay stops like whale safari stay visible. Co-located quay pins (ferry + harbour + activity) are fan-out nudged on the map for readable labels only — routing/GPX keep true coords. Detours (Bøvær, Hesten/Fjordgård, Stø, Stokmarknes museum, …) must include coords + an `after` anchor.
